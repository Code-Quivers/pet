...
import { useDeleteProductQAMutation } from "@/redux/features/productQAApi";
import { useDeletePromoMutation } from "@/redux/features/promoCodeApi";
import { useDeleteAdvertisementMutation } from "@/redux/features/adAPi";

// ! isOpenDelete={isOpenDelete}

const AdvertisementDeleteModal = ({
  isOpenDelete,
  handleCloseDelete,
  deleteData,
}: any) => {
  const [deleteAdvertisement, { data, isSuccess, isError, isLoading, error, reset }] =
    useDeleteAdvertisementMutation();

  const toaster = useToaster();

  const handleDelete = async () => {
    await deleteAdvertisement({ adId: deleteData?.adId });
  };
      ...
      toaster.push(
        <Message bordered showIcon type="error" closable>
          <h4 className="font-semibold ">
            {
              // @ts-ignore
              error?.message || "Advertisement Delete Failed"
            }
          </h4>
        </Message>,
        { placement: "topEnd", duration: 2000 }
        ...
        <Modal.Body className="p-3 ">
          ...
          <div className=" ">
            ...
            <div className="mt-5 border p-5 border-black/10 rounded-lg">
              <h2>Ad Name : {deleteData?.adTitle}</h2>
            </div>
            <div className="p-3  mt-5 bg-[#feeddd] rounded-lg ">
              ...
              <p className="text-[#b53a07] text-sm font-medium">
                By deleting this Advertisement , you will not be able to undo
                this action
              </p>
            </div>
