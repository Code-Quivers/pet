...
import { useDeleteTaxMutation } from "@/redux/features/taxApi";
...
const DeleteTaxModal = ({
  ...
  const [deleteTax, { data, isSuccess, isError, isLoading, error, reset }] =
    useDeleteTaxMutation();

  const toaster = useToaster();

  const handleDelete = async () => {
    await deleteTax({ taxId: deleteData?.taxId });
  };
        ...
        <Modal.Body className="p-3 ">
          ...
          <div className=" ">
            ...
            <div className="mt-5 border p-5 border-black/10 rounded-lg">
              <h2>State Name : {deleteData?.state}</h2>
            </div>
