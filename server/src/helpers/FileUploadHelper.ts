...
const testimonialStorage = multer.diskStorage({
  destination: function (req, file, callback) {
    callback(null, 'uploads/testimonialImg/');
  },
  filename: function (req, file, cb) {
    const uniqueFilename = Date.now() + '-' + file.originalname;
    cb(null, uniqueFilename);
...
const uploadTestimonialImage = multer({
  storage: testimonialStorage, // Use storageForTackPack instead of undefined storage
  limits: { fileSize: 5 * 1024 * 1024 }, // 5 mb
...
const uploadBlogImage = multer({
  storage: blogStorage, // Use storageForTackPack instead of undefined storage
